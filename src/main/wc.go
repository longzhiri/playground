...
func Map(value string) *list.List {
	ss := strings.FieldsFunc(value, func(r rune) bool {
		return !unicode.IsLetter(r)
	})

	l := list.New()
	for _, word := range ss {
		l.PushBack(mapreduce.KeyValue{Key: word, Value: strconv.FormatUint(1, 10)})
	}
	return l
}
...
func Reduce(key string, values *list.List) string {
	return strconv.FormatUint(uint64(values.Len()), 10)
}
